                    ...
                    try:
                        async with session.get(img, headers=headers) as img_res:
                            ...
                            if not os.path.exists(path):
                                os.mkdir(path)
                            with open(path + "\knife_fengniao_01_{}".format(img.split('/')[-1]), 'wb') as f:
                                f.write(imgcode)
                                f.close()
...
if __name__=="__main__":
    ...
    url_format = 'https://www.veer.com/search-image/shouwowandao/?page={}'
    full_urllist = [url_format.format(i) for i in range(1, 3)]
