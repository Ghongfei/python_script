...
if __name__ == '__main__':
    dst_dir = '/home/ai/kh/coco/data'
    images_dir = os.path.join(dst_dir, 'JPEGImages') # data/images
    labels_dir = os.path.join(dst_dir, 'labels') # data/labels
    dataDir = '/home/ai/kh/coco/coco2014'  # COCO数据集所在的路径
    dataType = 'train2014'  # 要转换的COCO数据集的子集名
    annFile = '%s/annotations/instances_%s.json' % (dataDir, dataType)  # COCO数据集的标注文件路径
    # classes = get_classes_and_index('/home/ai/kh/coco/extract_list_2.txt')  # {'car': '0'}
    # classes = {'car': '0'}
    classes = {'person': '1'}
    ...
    for imgId in imgIds:
        ...
        if objCount > 0:
            ...
            src_img = os.path.join(dataDir, '%s/%s/%s/%s' % (dataType, dataType, dataType, filename))
            dst_img = os.path.join(images_dir, filename)
            # shutil.copy(src_img, dst_img)
            # list_file.write('data/images/%s\n' % filename) # 相对路径
            list_file.write(dst_img + '\n')  # 绝对路径
